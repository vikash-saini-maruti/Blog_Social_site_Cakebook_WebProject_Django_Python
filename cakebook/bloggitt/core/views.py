...
def postlistnew(request):
    ...
    return render(request,'index.html', {"post2": post2})
    # return render(request, "index.html")
...
def postdetail(request, slug):
    if not request.user.is_authenticated:
        ...
        post.read_count += 1
        post.save()
        return render(request, 'detailpublic.html', {'post': post,'comments' : comments })
...
def Why_us(request):
    context={}
    return render(request,'Why_us.html',context=context)
def Trading(request):
    context={}
    return render(request,'Trading.html',context=context)
